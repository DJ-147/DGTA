...
class NodeFormerConv(nn.Module):
    def __init__(self, in_channels, out_channels, num_heads,
                 ...
                 rb_order=0, rb_trans='sigmoid', use_edge_loss=True,
                 topk_ratio=0.1):
        ...
        if rb_order >= 1:
            self.b = nn.Parameter(torch.FloatTensor(rb_order, num_heads))
        ...
        self.topk_ratio = topk_ratio  #  新增采样比例（降低复杂度）

    def reset_parameters(self):
        for w in [self.Wk, self.Wq, self.Wv, self.Wo]:
            w.reset_parameters()
        if self.rb_order >= 1:
    ...
    def forward(self, z, adjs, tau):
        ...
        # 轻量化随机投影（固定随机矩阵，减少重建代价）
        if self.projection_matrix_type is not None:
            dim = query.shape[-1]
            projection_matrix = torch.randn(self.nb_random_features, dim, device=z.device) / (dim ** 0.5)
        else:
            projection_matrix = None

        # ---- 核心优化点：Top-k 稀疏注意力替代全连接 ----
        # 计算 query-key 相似度，只保留 top-k
        with torch.no_grad():
            k = max(1, int(self.topk_ratio * N))
            sim = torch.einsum('bnhd,bmhd->bhnm', query, key) / (self.out_channels ** 0.5)
        ...
        # ---- message passing ----
        z_next = torch.einsum('bhnm,bmhd->bnhd', attn, value)
        ...
        # ---- edge regularization loss (轻量化) ----
        if self.use_edge_loss:
            row, col = adjs[0]
            d_in = degree(col, N, dtype=torch.float).to(z.device)
            d_norm = 1.0 / (d_in[col] + 1e-6)
            d_norm_ = d_norm.view(1, -1, 1).repeat(1, 1, self.num_heads)
            link_loss = (attn.mean(dim=-2).log() * d_norm_).mean()
...
class DGTA(nn.Module):
    '''
    DGTA model implementation, based off Nodeformer
    '''
